        ...
        <>
          ...
          >
            <div className={`min-w-0 overflow-hidden rounded-xl border border-line bg-panel/50 ${isFullscreen ? "flex min-h-[70vh] flex-col 2xl:h-[calc(100vh-2rem)]" : ""}`}>
              <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-3 border-b border-line bg-shell px-3 py-3">
                <div className="flex min-w-0 basis-56 grow items-start gap-2 text-xs leading-relaxed text-muted"><Move size={16} className="mt-0.5 shrink-0" aria-hidden="true" /><span>{t("Drag canvas to pan · drag nodes to arrange", "ಪ್ಯಾನ್ ಮಾಡಲು ಕ್ಯಾನ್ವಾಸ್ ಎಳೆಯಿರಿ · ಜೋಡಿಸಲು ನೋಡ್‌ಗಳನ್ನು ಎಳೆಯಿರಿ")}</span></div>
                <div className="flex shrink-0 items-center gap-1" role="group" aria-label={t("Graph zoom controls", "ಜಾಲ ನಕ್ಷೆ ಜೂಮ್ ನಿಯಂತ್ರಣಗಳು")}>
                  <button type="button" onClick={() => changeZoom(-ZOOM_STEP)} disabled={zoom <= MIN_ZOOM} className="grid h-11 w-11 place-items-center rounded-lg border border-line bg-panel text-inherit hover:border-brand focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand disabled:cursor-not-allowed disabled:opacity-40" aria-label={t("Zoom out", "ಜೂಮ್ ಔಟ್")} title={t("Zoom out", "ಜೂಮ್ ಔಟ್")}><ZoomOut size={18} aria-hidden="true" /></button>
                  <span className="min-w-12 text-center text-xs font-semibold text-inherit" aria-live="polite">{Math.round(zoom * 100)}%</span>
                  <button type="button" onClick={() => changeZoom(ZOOM_STEP)} disabled={zoom >= MAX_ZOOM} className="grid h-11 w-11 place-items-center rounded-lg border border-line bg-panel text-inherit hover:border-brand focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand disabled:cursor-not-allowed disabled:opacity-40" aria-label={t("Zoom in", "ಜೂಮ್ ಇನ್")} title={t("Zoom in", "ಜೂಮ್ ಇನ್")}><ZoomIn size={18} aria-hidden="true" /></button>
                  <button type="button" onClick={() => setIsFullscreen((current) => !current)} aria-pressed={isFullscreen} className="grid h-11 w-11 place-items-center rounded-lg border border-line bg-panel text-inherit hover:border-brand focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand" aria-label={isFullscreen ? t("Exit full screen", "ಪೂರ್ಣ ಪರದೆಯಿಂದ ನಿರ್ಗಮಿಸಿ") : t("Open full screen", "ಪೂರ್ಣ ಪರದೆ ತೆರೆಯಿರಿ")} title={isFullscreen ? t("Exit full screen", "ಪೂರ್ಣ ಪರದೆಯಿಂದ ನಿರ್ಗಮಿಸಿ") : t("Open full screen", "ಪೂರ್ಣ ಪರದೆ ತೆರೆಯಿರಿ")}>{isFullscreen ? <Minimize2 size={18} aria-hidden="true" /> : <Maximize2 size={18} aria-hidden="true" />}</button>
                  <button type="button" onClick={resetLayout} className="grid h-11 w-11 place-items-center rounded-lg border border-line bg-panel text-inherit hover:border-brand focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand" aria-label={t("Reset graph", "ಜಾಲ ನಕ್ಷೆಯನ್ನು ಮರುಹೊಂದಿಸಿ")} title={t("Reset graph", "ಜಾಲ ನಕ್ಷೆಯನ್ನು ಮರುಹೊಂದಿಸಿ")}><RotateCcw size={18} aria-hidden="true" /></button>
                </div>
              </div>
